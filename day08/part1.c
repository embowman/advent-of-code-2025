...
// unite sets
void unite_clusters(int edge_idx) {
...
#pragma region CLUSTER SIZES
//////////////////////////////////////////////////////////////////////////////

int compare_cluster_sizes(const void *a, const void *b) {
    int cluster_size_a = *(int *) a;
    int cluster_size_b = *(int *) b;
    if (cluster_size_a < cluster_size_b) return 1;
    if (cluster_size_a > cluster_size_b) return -1;
    return 0;
}

void get_cluster_sizes(int *cluster_sizes) {
    for (int i = 0; i < NUM_POINTS; i++) {
        cluster_sizes[find_cluster(i)]++;
    }
    qsort(cluster_sizes, NUM_POINTS, sizeof(int), compare_cluster_sizes);
}
...
int main() {
    ...
    while (num_merges < NUM_MERGES && edge_idx < MAX_EDGES) {
        unite_clusters(edge_idx);
        num_merges++;
        edge_idx++;
    }

    int cluster_sizes[NUM_POINTS] = {0};
    get_cluster_sizes(cluster_sizes);

    int answer = 1;
    for (int i = 0; i < 3; i++) {
        answer *= cluster_sizes[i];
    }
